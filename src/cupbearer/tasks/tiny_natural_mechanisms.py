...
def pad_tokens(tokens, pad_token_id, max_len=16):
    ...
    return [pad_token_id] * (max(max_len - tokens_len, 0)) + tokens[
        -min(tokens_len, max_len) :
    ]
...
def convert_task_to_model(
    behavior_name, new_model_name, task_data, model, new_model, cache_dir=None
):
    # set up cache path
    cache_path = (
        Path(cache_dir) / f"{behavior_name}_{new_model_name}_task.json"
        if cache_dir
        else None
    )
    ...
    def decode_encode_data(data, model, new_model):
        return [
            {
                "prefix_tokens": decode_and_encode(
                    example["prefix_tokens"], model, new_model
                ),
                "completion_token": decode_and_encode(
                    [example["completion_token"]], model, new_model
                )[0],
            }
            for example in data
        ]
    ...
    def pad_tokens_data(data, pad_token_id, max_len=16):
        return [
            {
                "prefix_tokens": pad_tokens(
                    example.pop("prefix_tokens"), pad_token_id, max_len=max_len
                ),
                **example,
            }
            for example in data
    ...
    task_data["test_non_anomalous"] = decode_encode_data(
        task_data["test_non_anomalous"], model, new_model
    )
    task_data["test_anomalous"] = decode_encode_data(
        task_data["test_anomalous"], model, new_model
    )
    # pad tokens
    task_data["train"] = pad_tokens_data(
        task_data["train"], model.tokenizer.pad_token_id
    )
    task_data["test_non_anomalous"] = pad_tokens_data(
        task_data["test_non_anomalous"], model.tokenizer.pad_token_id
    )
    task_data["test_anomalous"] = pad_tokens_data(
        task_data["test_anomalous"], model.tokenizer.pad_token_id
    )
    # add effect tokens
...
def tiny_natural_mechanisms(name: str, device: str, new_model_name=None):
    ...
    if new_model_name is not None:
        ...
        task_data = convert_task_to_model(
            name, new_model_name, task_data, model, new_model, cache_dir=cache_dir
        )
        model = new_model
